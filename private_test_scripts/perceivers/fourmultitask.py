...
trains4,valid4,test4=get_dataloader(1,32,5)
...
for i in range(1):
    ...
    model.to_logitslist=torch.nn.ModuleList([torch.nn.Sequential(torch.nn.LayerNorm(128),torch.nn.Linear(128,2)),torch.nn.Sequential(torch.nn.LayerNorm(128),torch.nn.Linear(128,10)),torch.nn.Sequential(torch.nn.LayerNorm(384),torch.nn.Linear(384,2)),torch.nn.Sequential(torch.nn.LayerNorm(384),torch.nn.Linear(384,2))]).to(device)

    from private_test_scripts.perceivers.train_structure_multitask import train

    records=train(model,80,[trains1,trains2,trains3,trains4],[valid1,valid2,valid3,valid4],[test1,test2,test3,test4],[['static','timeseries'],['colorlessimage','audiospec'],['feature1','feature2','feature3'],['feature4','feature5','feature3']],'private_test_scripts/perceivers/fhundricmic30.pt',lr=0.0008,device=device,train_weights=[1.2,0.9,1.1,1.0],is_affect=[False,False,False,False],unsqueezing=[True,False,False,False],transpose=[False,False,False,False],evalweights=[1,1,1,1],start_from=0,weight_decay=0.001)
