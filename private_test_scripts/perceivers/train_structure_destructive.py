...
def train(model,epochs,trains,valid,test,modalities,savedir,lr=0.001,weight_decay=0.0, optimizer=torch.optim.Adam, criterion=torch.nn.CrossEntropyLoss(),unsqueezing=[True,True], device="cuda:0",train_weights=[1.0,1.0],is_affect=[False,False],transpose=[False,False],using=[False,False]):
    ...
    for ep in range(epochs):
        ...
        for js in fulltrains:
            ...
            for ii in js:
                ...
                out=model(indict)
                loss=criterion(out,(1+js[ii][-1].long().to(device)) % len(out[0]))
        ...
        for ii in range(len(trains)):
            print("epoch "+str(ep)+" train loss dataset " +str(ii)+":" )
        """
        with torch.no_grad():
            accs=0.0
